...
class WebhooksCoinbaseController extends Controller {
    ...
    public function __invoke(Request $request) {
        ...
        try {
            ...
            if ($event->data['name'] == 'XXXXXX') { // Eucatigre / Kintun Expediciones
                ...
                if ($event->type == 'charge:confirmed') {
                    ...
                    if ($reserva->emailSend == 0) { //emailSend en 1 significa email ya enviado
                        ...
                        //remitente
                            $message->from('noreply@xxxxxx.com', 'xxxxxx.com');
                        //asunto
                            $message->subject('RESERVA xxxxxx xxxxxx');
                        //destinatarios
                            $destinatarios = explode(',', $config->destinatarios);
                            foreach ($destinatarios as $destinatario) {
                                $message->to($destinatario, 'xxxxxx.com');  
                            }
                ...
                } // endif charge:confirmed

        }  // fin si es el cliente
